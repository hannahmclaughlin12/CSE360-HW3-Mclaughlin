...
public class PasswordEvaluator {
	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			} else if ("~`!@#$%^&*()_-+{}[]|:,.?/".indexOf(currentChar) >= 0) { // Updated to reflect accepted special characters
				System.out.println("Special character found");
				foundSpecialChar = true;
			} else { // Checks if there are any invalid characters
				otherChar = true;
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "an upper case letter";
		
		if (!foundLowerCase) {
			if (errMessage != "") {
				errMessage += ", ";
			}
			errMessage += "a lower case letter";
		}
		...
		if (!foundSpecialChar) {
			...
			errMessage += "a special character";
		}
		...
		passwordIndexofError = currentCharNdx;
		return "Your password needs " + errMessage;
